...
public class EnemySpawner : MonoBehaviour
{
    ...
    private void SpawnEnemy()
    {
        ...
        {
            enemyInstance = PoolManager.Instance.Get(prefabToSpawn, spawnPoint.position, spawnPoint.rotation);
        }
        if (enemyInstance == null)
        {
            enemyInstance = Instantiate(prefabToSpawn, spawnPoint.position, spawnPoint.rotation);
            var po = enemyInstance.GetComponent<PooledObject>();
            if (po == null) po = enemyInstance.AddComponent<PooledObject>();
            po.SourcePrefab = prefabToSpawn;
        }
        if (enemyInstance.GetComponent<WF.Gameplay.Systems.Buffs.BuffManager>() == null)
        {
            enemyInstance.AddComponent<WF.Gameplay.Systems.Buffs.BuffManager>();
        }
        ...
        combatController.OnDefeated += () =>
        {
            ...
            if (PoolManager.Instance != null)
            {
                PoolManager.Instance.Release(enemyInstance);
            }
            else
            {
                Destroy(enemyInstance);
